"""Tests for Nuxt Scraper anti_detection module utilities."""
...
async def test_simulate_mouse_movement_moves_mouse(mock_page: object) -> None:
    mock_page.locator.return_value.bounding_box = AsyncMock(
...
async def test_human_type_simulates_typing(mock_page: object) -> None:
    with patch(
        "nuxt_scraper.anti_detection.typing.human_delay", new_callable=AsyncMock
    ) as mock_delay:
...
def test_stealth_scripts_are_list_of_strings() -> None:
    ...
    # At least one script should contain common stealth patterns
    combined = " ".join(STEALTH_SCRIPTS)
    assert "webdriver" in combined or "navigator" in combined or "chrome" in combined
